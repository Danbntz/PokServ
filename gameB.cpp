...
bool Game::placeCreature(Creature* creature, const Position& pos, bool extendedPos /*= false*/, bool forced /*= false*/)
{
	...
	if (tmpPlayer && !checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
    {
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	if((toTile = map->getTile(destPos)))
		//if(!getTile(destPos)->isHouseTile())
		...
		{ 
			...
			if(thing->getItem() && toItem->getID() >= 1) 
			{
				...
				{ 
                    ...
                    //tileFFly->updateTileFlags(toItem, true);
                    	//tileFFly->onUpdateTile();
					//if(map->getTile(destPos) && map->getTile(destPos)->__getThing(0) && map->getTile(destPos)->__getThing(0)->getItem() &&
					//map->getTile(destPos)->__getThing(0)->getItem()->getID() != 460)//(460 || 11676 || 11675 || 11677))
	...
	/*std::string flySto;
    if (creature->getStorage(17000, flySto) && flySto != "0" && !map->getTile(destPos)->ground) //está de fly
    {
    int32_t flyIntValue = atoi(flySto.c_str());
    if (flyIntValue > 0){
                Tile* checkTile = map->getTile(destPos);
                CombatParams params;
                params.combatType = COMBAT_NONE;
                params.effects.impact = MAGIC_EFFECT_NONE;
                CombatArea* area = new CombatArea();
                area->setupArea(5);
                const ItemType& it = Item::items[460];
                Combat::doCombatHealth(creature,creature->getPosition(), area, 0, 0, params);
                //adicionar item se não existir
                if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) {
                 Item* newItem = Item::CreateItem(460, 1);
                 Tile* tile = checkTile;
                 internalAddItem(NULL, tile, newItem, INDEX_WHEREEVER, FLAG_NOLIMIT);
                }
                //
	...
	if(ret != RET_NOERROR)
	{
		if(Player* player = creature->getPlayer())
		{
			...
			if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1) && !map->getTile(destPos))
			{
				...
				//internalMoveCreature(NULL, creature, fromTile, toTile, flags);
				...
				if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
                {
                    ...
                    internalAddItem(NULL, tileF, newItemF, INDEX_WHEREEVER, FLAG_NOLIMIT);
                    //tileF->onUpdateTile();
					//internalMoveCreature(creature->getPlayer()->getCreature(), (Direction)direction, FLAG_NOLIMIT);
							...
							if(thing->getItem() && toItem->getID() >= 1) 
							{
								if(toItem->getID() == 460 || toItem->getID() == 11676 || toItem->getID() == 11675 || toItem->getID() == 11677)
					    		{
									internalRemoveItem(NULL, thing->getItem());
                   	    			...
                   	    			//tileFFly->onUpdateTile();
									if(retF == RET_NOERROR)
									    onUpdateFlyTile(fromTile->getPosition());//player->sendUpdateTileFly
								}
							}
						}
                    }
						...
						if(thing)
				 		{
							...
							if(thing->getItem() && toItem->getID() >= 1) 
							{
								if(toItem->getID() == 460 || toItem->getID() == 11676 || toItem->getID() == 11675 || toItem->getID() == 11677)
					    		{
                                    ...
                                    // else
										//tileFFly->onUpdateTile();
						//onUpdateFlyTile(fromTile->getPosition());//ultimoteste
				...
				/*if (!getTile(destPos))
				{
                    Item* newItem = Item::CreateItem(11677);
			        Tile* tile = IOMap::createTile(newItem, NULL, destPos.x, destPos.y, destPos.z);
				    setTile(tile);
				    tile->onUpdateTile();
				}
				...
				if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
                {
                    Item* newItemF = Item::CreateItem(11677, 1);
                    Tile* tileF = checkTile;
                    internalAddItem(NULL, tileF, newItemF, INDEX_WHEREEVER, FLAG_NOLIMIT);
                    tileF->onUpdateTile();
					internalMoveCreature(creature->getPlayer()->getCreature(), (Direction)direction, FLAG_NOLIMIT);
							...
							if(thing->getItem() && toItem->getID() >= 1) 
							{
								if(toItem->getID() == 460 || toItem->getID() == 11676 || toItem->getID() == 11675 || toItem->getID() == 11677)
					    		{
									internalRemoveItem(NULL, thing->getItem());
                   	    			...
                   	    			//tileFFly->onUpdateTile();
									//if(retF == RET_NOERROR)
									    onUpdateFlyTile(fromTile->getPosition());//player->sendUpdateTileFly(fromTile->getPosition());
								}
							}
						}
                	}
...
/*	if(fromPos.x == 65535 && fromPos.y == 10 && fromPos.z == 0 && toPos.x == 65535 && toPos.y == 8 && toPos.z == 0 
    || fromPos.x == 65535 && fromPos.y == 8 && fromPos.z == 0  && toPos.x == 65535 && toPos.y == 10 && toPos.z == 0)
    {
        Thing* athing = player->__getThing(8);
        Thing* bthing = player->__getThing(10);
		
    	if(athing && bthing && !athing->getItem()->isNotMoveable() && !bthing->getItem()->isNotMoveable())
        {
       
    ///
            boost::any bbpb1 = athing->getItem()->getAttribute("poke");
		    boost::any bbpb2 = bthing->getItem()->getAttribute("poke");  
            if (!bbpb1.empty() && !bbpb2.empty())
            {
                     boost::any abMBt = athing->getItem()->getAttribute("moveBallT");
            boost::any bbMBt = bthing->getItem()->getAttribute("moveBallT");
	if(abMBt.type() == typeid(std::string) || bbMBt.type() == typeid(std::string))
	{
	 	std::string apBMBt = boost::any_cast<std::string>(abMBt);
	 	std::string bpBMBt = boost::any_cast<std::string>(bbMBt);
	 	if(apBMBt == "use" || bpBMBt == "use")
	 	{
	 	    player->sendTextMessage(MSG_INFO_DESCR, "sorry, not possible.");
	 	    return false;
        }
    }				
                Cylinder* acylinder = athing->getItem()->getParent();
                Cylinder* bcylinder = bthing->getItem()->getParent();
                int32_t aitemIndex = acylinder->__getIndexOfThing(athing->getItem());
                int32_t bitemIndex = bcylinder->__getIndexOfThing(bthing->getItem());
                internalRemoveItem(NULL, athing->getItem());
                internalRemoveItem(NULL, bthing->getItem());
             
                Item* aa = Item::CreateItem(athing->getItem()->getID(), athing->getItem()->getItemCount());
                aa->copyAttributes(athing->getItem());
	...
	if(valueU.type() == typeid(int32_t))
	{
        if(boost::any_cast<int32_t>(valueU) == 1)
        {
            ...
            }else if(thingT && thingT->getItem())
            {
			    ...
			    if(!itemT->isDepot() && itemT->isContainer() && itemT->getID() != 1987)
				{
					player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			    	return false;
				}
                ...
                { 
                    ...
                    if(!itemTC->isDepot() && itemTC->isContainer() && itemTC->getID() != 1987)
				    {
					    player->sendTextMessage(MSG_INFO_DESCR, "It is a Unique Item, you can't throw away this item!");
			            return false;
				    }
...
void Game::addMoneyMarket(std::string name, int64_t money)
{
	...
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				Item* remaindItem = Item::CreateItem(it->second-1, std::min((int64_t)100, tmp));
				IOLoginData::getInstance()->playerMail(NULL, name, 1, remaindItem);
...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if(attackCreature->getNpc())
	{
		...
		attackCreature->getNpc()->onCreatureSay(creature, type, text, pos);
		player->setAttackedCreature(NULL);
		player->sendCancelTarget();
		return false;
	}
...
bool Game::doPlayerAddItemStacking(Player* player, Container* container, uint16_t itemid, uint8_t quant) 
{
	...
	//Container* container = thinc->getCointainer();
	for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
    //for(uint8_t slot = 0; slot <= container->size(); slot++)
    {
        ...
        if(item && item->getID() == itemid)
        {
            ...
            {
            	uint8_t count = item->getItemCount()+quant;
            	if(count > 100)
				{
				   quant = count-100;
            	   count = 100;  
            	...
            	/*if(quant > 100)
            	{
            		Item* newItem = Item::CreateItem(item->getID(), (item->getItemCount()+quant)-100);
