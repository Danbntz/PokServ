...
void Game::addMoney(Cylinder* cylinder, int64_t money, uint32_t flags /*= 0*/)//viktorgold
{
	...
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				/*Player* player = cylinder->getPlayer();
				if(player && player->__getThing(3))
				{
					Thing* bagT = player->__getThing(3);
					Item* bag = bagT->getItem();
					if(bag->getContainer())
					{
						Container* container = bag->getContainer();
						uint8_t quant = std::min((int64_t)100, tmp);
						for(ContainerIterator itB = container->begin(), end = container->end(); itB != end; ++itB)
						{
    						Item* item = (*itB)->getItem();
				    		if(item && item->getID() == it->second)
        		    		{
						...
						if(quant > 0)
						{
							Item* newItem = Item::CreateItem(it->second, quant);
...
bool Game::requestSellMarket(uint32_t playerId, const Position& pos, int16_t stackpos, uint16_t spriteId)
{
	...
	if(!marketItem || marketItem->getClientID() != spriteId || !marketItem->isPickupable() || marketItem->isNotMoveable() ||
        (marketItem->isLoadedFromMap() && (marketItem->getUniqueId() != 0 || (marketItem->getActionId() != 0 && marketItem->getContainer()))))
	{
		//player->sendCancelMessage(RET_NOTPOSSIBLE);
		player->sendExtendedOpcode(155, "Desculpe, mas você não pode vender este item."); 
		return false;
	}
	...
	{
		player->sendExtendedOpcode(155, "Desculpe, mas você não pode vender bag com itens dentro.");
		return false;
	}
...
bool Game::acceptSellMarket(uint32_t playerId, uint16_t count, uint64_t price, bool type_money, bool type_unity)
{
	...
	{
		player->sendExtendedOpcode(155, "Venda concluida.");
	}
