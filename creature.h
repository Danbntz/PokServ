...
class Creature : public AutoId, virtual public Thing
{
	...
	public:
    	...
    	virtual const std::string& getNickss() const 
		{
			std::string str = "";
        	if (!ditto.empty())
        		str = " (" + ditto + ")";
		...
		void setHideHealth(bool v) {hideHealth = v;}
			void setStatValue(std::string stat, int32_t value)
	{
      Stats[stat] = value;
	}
		void setNick(std::string newNick);
		void setIDPad(int32_t idpoke) {  
        idpad = idpoke;
        }
		void setPDuel(uint8_t duel) {  
        pduel = duel;
        }
        void setGym(bool gyn) {  
        gym = gyn;
        }
        void setOrderPos(const Position& pos) {
        orderPos = pos;     
        }
        void setOrderHabil(std::string habi) {
        orderHabil = habi;     
        }
		void setLastNick(std::string newNick) {  
        lastNick = newNick;
        }
