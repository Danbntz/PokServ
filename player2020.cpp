 	...
 	for(int32_t i = SLOT_LAST-1; i >= SLOT_FIRST; --i)
	{
		...
		{ 
            ...
            if(value.type() == typeid(std::string))
            {
				...
				{
                    boost::any levepo = (item)->getAttribute("123001");
                    ...
                    {
						s << "," << foto->clientId << "|" << "[" << boost::any_cast<int32_t>(level) << "] " << boost::any_cast<std::string>(value) << "|" << boost::any_cast<int32_t>(border) << "|" << boost::any_cast<int32_t>(sexo) << "|" << hpPok;
                ...
                for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
      	        {
                    ...
                    if(value.type() == typeid(std::string))
                    {
				        ...
				        if(tapot.type() == typeid(int32_t))
				        {
							boost::any levepo = (*it)->getAttribute("123001");
                            ...
                            if(border.type() == typeid(int32_t))
                            {
								...
								//pokeBarList.insert(boost::any_cast<int32_t>(border), s);
								std::string tx = s.str();
					            pokeBarList[boost::any_cast<int32_t>(border)] = tx;
					        	s.str("");
                				bo += 1; 
...
void Player::setPlayerCatchCount(std::string _poke, uint8_t typec, uint16_t count) 
{
    ...
    if(it == playerCatch.end())
    {
        playerCatch[_poke] = catchPlayer_t(_poke, false, false, false, 0, 0, 0, 0, 0, 0, 0); 
...
void Player::setPlayerCatch(std::string _poke, bool _dex, bool _use, bool _catch)
{
    ...
    playerCatch[_poke] = catchPlayer_t(_poke, _dex, _use, _catch, 0, 0, 0, 0, 0, 0, 0);
}
