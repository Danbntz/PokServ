...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert)
{
	...
	while(stackList.size() > 0)
	{
		...
		{
			//if(i == 0)
			//	std::cout << "**SAVE ITEM SYSTEM DB** CONTAINER " << player->getName() << " SIZE " << container->size() << std::endl;
			
			item = container->getItem(i); 
			if(!item)
			{
				std::cout << "**SAVE ITEM SYSTEM DB** NAO ER ITEM PLAYER " << player->getGUID() << " ITEMID " << i << std::endl;	
				continue;
			}
            ...
            std::cout << "**SAVE ITEM SYSTEM DB** " << player->getGUID() 
			<< ", SIZE " << container->size() 
            << ", " << stack.second
			<< ", " << runningId
			<< ", " << item->getID()
			...
			if(!query_insert.addRow(buffer)) 
			{
				std::cout << "**SAVE ITEM SYSTEM DB** NAO SALVOU ITEM PLAYER " << player->getGUID() << " ITEMID " << item->getID() << std::endl;	
				return false;
			}
		}
	}
