...
void ProtocolGame::GetTileDescription(const Tile* tile, NetworkMessage_ptr msg)
{
	...
	if(tile->ground)
	{
		...
		count++;
	}/*else if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1))
	{   
        msg->AddItemId(11677);
	    count++;
	    return;
	...
	for(int32_t nx = 0; nx < width; nx++)
	{
		for(int32_t ny = 0; ny < height; ny++)
		{
			...
			if(tile)
			{
				...
				skip = 0;
				//Thing* thing = tile->__getThing(0);
				//if(thing) 
				//{
					//Item* toItem = thing->getItem();
					//if(thing->getItem() && toItem->getID() >= 1) 
				
				//if(player->getPosition().z <= 6 && z <= 6 && thing && thing->getItem() && thing->getItem()->getID() >= 1)
				if(!tile->hasProperty(BLOCKSOLID) && !tile->ground && player->getPosition().z <= 6 && player->getStorage(17000, value) && atoi(value.c_str()) >= 1)// || !tile->ground)
			...
			{
				//if(g_game.getTile(Position(x + nx + offset, y + ny + offset, z)))
    		    //{
				/* viktor Position pos = Position(x + nx + offset, y + ny + offset, z);//g_game.getTile(Position(x + nx + offset, y + ny + offset, z));
				...
				//tileFly = new StaticTile(x + nx + offset, y + ny + offset, z);
			     //   g_game.setTile(tileFly);
				//tile->onUpdateTile();
				//}
				if(skip >= 0)
				{
				...
				Tile* tileFly = g_game.getTile(Position(x + nx + offset, y + ny + offset, z));
				if(!tileFly)
				{
				    tileFly = new StaticTile(x + nx + offset, y + ny + offset, z);
			        g_game.setTile(tileFly);
                }
                //GetTileDescription(tileFly, msg);
				GetTileDescription(tileFly, msg);*/
			}
			else
			{
...
void ProtocolGame::sendMoveCreature(const Creature* creature, const Tile* newTile, const Position& newPos,
	uint32_t newStackpos, const Tile* oldTile, const Position& oldPos, uint32_t oldStackpos, bool teleport)//Viktor TV
{
	if(creature == player)
	{
		NetworkMessage_ptr msg = getOutputBuffer();
