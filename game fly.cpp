	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	if(item->isStackable())
	{
		...
		{
			/*boost::any vUi = (item)->getAttribute("unico");
			boost::any vUTi = (toItem)->getAttribute("unico");
	        if(vUi.type() == typeid(int32_t) && vUTi.type() == typeid(int32_t))
	        { 
				if(boost::any_cast<int32_t>(vUi) == 1 && boost::any_cast<int32_t>(vUTi) == 1)
				{
					n = std::min((uint32_t)100 - toItem->getItemCount(), m);
					toCylinder->__updateThing(toItem, toItem->getID(), toItem->getItemCount() + n);
					updateItem = toItem;
				}
			}else if(vUi.empty() && vUTi.empty())
			{
				n = std::min((uint32_t)100 - toItem->getItemCount(), m);
				toCylinder->__updateThing(toItem, toItem->getID(), toItem->getItemCount() + n);
				updateItem = toItem;
