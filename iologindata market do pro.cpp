...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	if((result = db->storeQuery(query.str())))
	{
    	...
    	uint16_t value;
    	bool item = propStreamList.GET_USHORT(value);
    	while (item) {
        	player->addAutoLootItem(value);
        	item = propStreamList.GET_USHORT(value);
    	}
	}
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	query_insert.setQuery("INSERT INTO `player_catch` (`player_id`, `poke`, `dex`, `use`, `catch`, `pokeball`, `greatball`, `superball`, `ultraball`, `saffariball`, `masterball`,`premierball`) VALUES ");
	
   	for(PlayerCatch::iterator itC = player->playerCatch.begin(); itC != player->playerCatch.end(); ++itC)
   	{
		sprintf(buffer, "%u, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d", player->getGUID(), db->escapeString(itC->second.poke).c_str(), itC->second.dex, itC->second.use, itC->second.catchP, itC->second.pokeball,itC->second.greatball,itC->second.superball,itC->second.ultraball,itC->second.saffariball,itC->second.masterball,itC->second.premierball);
		if(!query_insert.addRow(buffer))
			return false;
    }
    if (!query_insert.execute())
...
uint64_t IOLoginData::getMoneyMarket(uint32_t id, bool type)
{
    Database* db = Database::getInstance();
	DBQuery query;
	...
	if((result = db->storeQuery(query.str())))
	{
		uint64_t money;
		if(type)
			money = result->getDataLong("market_d");
		else
			money = result->getDataLong("market_p");
...
bool IOLoginData::removeMoneyMarket(uint32_t id, bool type, uint64_t price)
{
	Database* db = Database::getInstance();
	DBQuery query;
	
	if(type)
		query << "UPDATE `players` SET `market_d` = " << getMoneyMarket(id, type)-price << " WHERE `id` = " << id << db->getUpdateLimiter();
	else
		query << "UPDATE `players` SET `market_p` = " << getMoneyMarket(id, type)-price << " WHERE `id` = " << id << db->getUpdateLimiter();
		
	return db->executeQuery(query.str());
}

bool IOLoginData::addMoneyMarket(uint32_t id, bool type, uint64_t value)
{
	Database* db = Database::getInstance();
	DBQuery query;
	
	if(type)
		query << "UPDATE `players` SET `market_d` = " << getMoneyMarket(id, type)+value << " WHERE `id` = " << id << db->getUpdateLimiter();
	else
		query << "UPDATE `players` SET `market_p` = " << getMoneyMarket(id, type)+value << " WHERE `id` = " << id << db->getUpdateLimiter();
...
bool IOLoginData::buyMarket(Player* player, uint16_t id, uint16_t count)
{
	...
	if((result = db->storeQuery(query.str())))
	{
        ...
        uint64_t money = (uint64_t)getMoneyMarket(player->getGUID(), moneyB);
        if(money < price)
        {
		...
		if(money >= price && buyItemMarket(player, result, count, id))
		{
            ...
            removeMoneyMarket(player->getGUID(), moneyB, (uint64_t)price);
			//g_game.addMoneyMarket(vendedor, price);
			addMoneyMarket(vendedorId, moneyB, (uint64_t)price);
...
bool IOLoginData::buyItemMarket(Player* player, DBResult* result, uint16_t count, uint16_t id, bool cancel)
{
	...
	PropStream propStream;
	propStream.init(attr, attrSize);
	if(Item* item = Item::CreateItem(result->getDataInt("itemtype")-1, count))
	{
		if(!item->unserializeAttr(propStream))
		{
...
bool IOLoginData::sellMarket(const Player* player, const Item* item, uint16_t count, uint64_t preco, bool money, bool stack)
{
	...
	query << ", " << db->escapeBlob(attributes, attributesSize) << ", " << time(NULL)+(7*24*60*60) << ", " << money << ", " << stack <<  ")";
