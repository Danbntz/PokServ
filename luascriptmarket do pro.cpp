...
//	char buffer[1024];
	//string
//	sprintf(buffer, "you see %s " , market["poke"], )
    std::stringstream s;
    if(market["poke"] != "")
	{
		...
		if(market["superboost"] != "")
	       s << " S.Boost: +" << market["superboost"];
	   
		if(market["heldx"] != "")
		{
	        s << " Helds: " << market["heldx"];
		    if(market["heldy"] != "")
		        s << " and " << market["heldy"];
			if(market["heldz"] != "")
		        s << " and " << market["heldz"];
		...
		if(market["vitamin"] != "")
	       s << " Vitamin: " << market["vitamin"];
	
		if(market["maestria"] != "")
	       s << " Maestria: " << market["maestria"];
	   
		if(market["nature"] != "")
	       s << " Nature: " << market["nature"];
	   
		if(market["serial"] != "")
  	     s << " Serial: " << market["serial"];
    }else{
    s << "you see " << market["nameitem"] << ".";
    s << market["viktor"] << " Níkãndo.";
}
    
	
//	s << "you see " << market["nameitem"] << " " << market["nick"] << " " << market["boost"] << " " << market["heldx"] << " " << market["heldy"];
   // char porra = s.str()
    //std::string vida = market["nameitem"];
   // if (!vida.empty())
       lua_pushstring(L, s.str().c_str());  
 	...
 	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		...
		{ 
            ...
            if(!value.empty())
            {
            	...
            	if (!money && IOLoginData::getInstance()->getMoneyMarket(player->getGUID(), money) < ((preco*count)-(uint64_t)((preco*count)*0.95))
                || money && IOLoginData::getInstance()->getMoneyMarket(player->getGUID(), false) < 1000000)
            	{
                    player->doEraseMarket();
        	    	lua_pushboolean(L, false); 
                	break;
            	...
            	if(player->__getItemTypeCount(itemMc->getID(), -1) >= count && IOLoginData::getInstance()->sellMarket(player, itemMc, count, preco,money,stack))
            	{
                   // g_game.internalRemoveItem(NULL, itemMc, count);//aqui 13 11 2018
                     if (money)
                         IOLoginData::getInstance()->removeMoneyMarket(player->getGUID(), false, 1000000);
                     else
                         IOLoginData::getInstance()->removeMoneyMarket(player->getGUID(), false, ((preco*count)-(uint64_t)((preco*count)*0.95)));
