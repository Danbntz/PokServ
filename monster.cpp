...
void Monster::onCreatureAppear(const Creature* creature)
{
	...
	if(creature == this)
	{
		...
		if(isSummon())
            ...
            if (this->getMonster() && this->getNick() == ""){
                std::string name = this->getName();
                std::string newNick;
                std::string myNick = this->getNick();
                if (name.find("Shiny ") != std::string::npos){
                    newNick = name.replace(name.find("Shiny "), 6, "");
...
bool Monster::doTeleportToMaster()
{
    ...
    /*g_game.getClosestFreeTile(this,getMaster()->getPosition(), false, false)*/
	if(g_game.internalTeleport(this, getMaster()->getPosition(), false) != RET_NOERROR)
		return false;
	...
	for(SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		if(canUseSpell(myPos, targetPos, *it, interval, inRange))
		{
			if(it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				//std::cout << it->isName.c_str() << std::endl;
				//it->spell->castSpell(this, attackedCreature);
				/*if(isGuardian() || !isSummon())
				{
                ...
                }else if(isSummon() && (it->isName == "melee" || it->isName == "physical")) 
				{// || it->isName.c_str() == "physical")){
					//InstantSpell* spellMove = g_spells->getInstantSpellByName(it->isName.c_str());
					//spellMove->castSpell(getCreature(), attackedCreature->getCreature(), it->isName.c_str());
					it->spell->castSpell(this, attackedCreature);
					//12/12/18 std::cout << getName() << " >>>> " << it->isName.c_str() << std::endl;
					//it->spell->castSpell(this, attackedCreature); 
				}*/
